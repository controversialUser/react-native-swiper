...
export default class extends Component {
  ...
  componentWillMount() {
    // if (Platform.OS !== 'ios') {
    //   const shouldSetResponder = (evt, gestureState) =>
    //     this.props.horizontal && Math.abs(gestureState.vx) > Math.abs(gestureState.vy);
  ...
  initState(props, updateIndex = false) {
    ...
    initState.offset[initState.dir] =
      initState.dir === 'y' ? height * initState.index : width * initState.index;

    // fix render last page first when loop = true
    if (props.loop) {
      initState.offset[initState.dir] =
        initState.dir === 'y' ? height * (initState.index + 1) : width * (initState.index + 1);
    }
